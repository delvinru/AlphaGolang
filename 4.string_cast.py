...
def is_string_load(addr):
    ...
    if (
        ...
        and not (
            "[" in idc.print_operand(addr, 1) or "loc_" in idc.print_operand(addr, 1)
        )
        and (
            (
                "offset " in idc.print_operand(addr, 1)
                or "h" in idc.print_operand(addr, 1)
            )
        ...
        addr_2 = ida_search.find_code(addr, ida_search.SEARCH_DOWN)
        try:
            dest_reg = idc.print_operand(addr_2, 0)[
                idc.print_operand(addr_2, 0).index("[")
                + 1 : idc.print_operand(addr_2, 0).index("[")
                + 4
            ]
        except ValueError:
            return False
        if (
            ...
            addr_3 = ida_search.find_code(addr_2, ida_search.SEARCH_DOWN)
...
def create_string(addr, string_len):
    ...
    if (
        ...
        and len(ida_bytes.get_strlit_contents(addr, string_len, STRTYPE_C))
        != string_len
    ):
    ...
    for addr in Functions(text_seg.start_ea, text_seg.end_ea):
        ...
        while addr <= end_addr:
            if is_string_load(addr):
                if "rodata" not in get_segm_name(addr) and "text" not in get_segm_name(
                    addr
                ):
                ...
                addr_3 = ida_search.find_code(
                    ida_search.find_code(addr, ida_search.SEARCH_DOWN),
                ...
                addr = ida_search.find_code(addr_3, ida_search.SEARCH_DOWN)
            else:
                addr = ida_search.find_code(addr, ida_search.SEARCH_DOWN)
