...
def is_simple_wrapper(addr):
    if (
        ...
        if (
            idc.print_insn_mnem(addr) == "jmp"
            and idc.print_operand(addr, 0) == "runtime_morestack"
        ):
            return True
...
def create_runtime_ms():
    ...
    runtime_ms_end = ida_bytes.find_bytes(
        opcodes,
        range_start=text_seg.start_ea,
        range_end=text_seg.end_ea,
    )
...
def create_pointer(addr, force_size=None):
    if force_size != 4 and (ida_ida.inf_is_64bit() or force_size == 8):
...
class _GoPclnTab:
    def __init__(
        self, start_ea, offset_funcdata, offset_nametab, functab_data_size=None
    ):
        self.addr = start_ea
        ...
        self.functab_data_size = (
            functab_data_size if functab_data_size is not None else self.ptr_size
        )
        tab_offsets_ea = self.addr + 8
        self.nfunctab = create_pointer(tab_offsets_ea, self.ptr_size)[0]

        self.text_start_ea, _ = create_pointer(
            tab_offsets_ea + 2 * self.ptr_size, self.ptr_size
        )
        if not idaapi.is_loaded(
            self.text_start_ea
        ):  # Sometimes, the value in the pcln tab isn't a valid adress
            self.text_start_ea = get_text_seg()

        self.funcnametab_ea = (
            self.addr
            + create_pointer(
                tab_offsets_ea + offset_nametab * self.ptr_size, self.ptr_size
            )[0]
        )
        self.funcdatatab_ea = (
            self.addr
            + create_pointer(
                tab_offsets_ea + offset_funcdata * self.ptr_size, self.ptr_size
            )[0]
        )
        self.cutab_ea = (
            self.addr
            + create_pointer(tab_offsets_ea + 4 * self.ptr_size, self.ptr_size)[0]
        )

    def _get_func_addr(self, idx):
        return create_pointer(
            self.funcdatatab_ea + 2 * idx * self.functab_data_size, self.ptr_size
        )[0]
    ...
    def _get_funcdata_ea(self, idx):
        return (
            self.funcdatatab_ea
            + create_pointer(
                self.funcdatatab_ea + (2 * idx + 1) * self.functab_data_size,
                self.ptr_size,
            )[0]
        )
    ...
    def _get_func_name_ea(self, idx):
        ...
        return self.funcnametab_ea + idaapi.get_dword(
            funcdata_ea + self.functab_data_size
        )
...
class GoPclnTab118(_GoPclnTab):
    ...
    def _get_func_addr(self, idx):
        return self.text_start_ea + idaapi.get_dword(
            self.funcdatatab_ea + 2 * idx * self.functab_data_size
        )

    def _get_funcdata_ea(self, idx):
        return self.funcdatatab_ea + idaapi.get_dword(
            self.funcdatatab_ea + (2 * idx + 1) * self.functab_data_size
        )
...
def parse_pcln(start_ea):
    ...
    if (
        magic == GO120_MAGIC
    ):  ## Go 120 has difference Magic, but executed same with GO118
        ...
        try:
            for func_ea, func_name in pcln.enumerate_functions():
                ...
                debug(
                    ...
                    % (func_ea, func_name, clean_func_name)
                )
                if idaapi.get_func_name(func_ea) is not None:
                    try:
                        ...
                        error(
                            "clean_func_name error %s, exception" % (clean_func_name, e)
                        )
                        continue
                    renamed += 1
        except Exception as e:
